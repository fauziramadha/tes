...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $mysqli->prepare("INSERT INTO tg_orders (orderId, customerID, customerName, customerAddress, productName, quantity, totalAmount, orderDate, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");

    if ($stmt === false) {
        die("Prepare failed: " . $mysqli->error);
    }

    $stmt->bind_param("sssssdsss", $newOrderId, $customerID, $customerName, $customerAddress, $productName, $quantity, $totalAmount, $orderDate, $status);
