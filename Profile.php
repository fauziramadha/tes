...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['submit'])) {        
        ...
        // Update the password in the database (replace with your database update logic)
...
<head>
    ...
    document.getElementById('password-form').addEventListener('submit', function(event) {
        ...
        .then(result => {
            // Reloading only necessary parts instead of entire body
    ...
    <aside>
        <div class="toggle">
            <div class="logo">
                ...
                <a href="index"><h2>Oven<span class="danger">Crust</span></h2></a>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        function validatePasswordForm() {
            ...
            // Password validation
            if (newPassword.length > 0) { // Only validate if new password is provided
                if (newPassword.length < minPasswordLength) {
                    showError('new-password-msg', `Password must be at least ${minPasswordLength} characters long.`);
                    isValid = false;
                } else {
            ...
            if (profilePicture) {
                if (!allowedTypes.includes(profilePicture.type)) {
                    showError('profile-picture-msg', 'Invalid file type. Only JPEG, PNG, and JPG are allowed.');
                    isValid = false;
                } else {
                    clearError('profile-picture-msg');
                }
            }
